...
由于带宽，流量大小都是对算法可见的，因此可以预测到每个重构时间内网络的所有动作，并能预测到下个重构时间的流量情况。这样预测会大幅提高算法复杂度，而且要预测两个重构时间，因此只能大致预测。

1. 开始调度时，算法对业务的训练和通信时间是不清楚的，需要靠多次迭代的数据来预测，业务训练时间可以看作常数，但是通信时间是随着带宽变化和本次迭代的网络状态而改变的。第一次迭代采用计算得到的初始拓扑，在该拓扑上对流进行调度。

2. 进行第一个拓扑重构时间内的调度：在业务通信需求开始时确定是否使用在网聚合：找到这个业务在前面算法获得的环拓扑，并检查网络中是否存存在这样的拓扑，如果不存在，那就直接放进非在网聚合队列；反之，如果存在，也要判断放进哪个队列可以更快完成：具体的做法为：计算队列中业务的总流量矩阵，除以聚合连接矩阵，找到最大非零项，即为完成该业务的时间；非聚合连接同理，但是需要加上该业务的聚合时间。聚合队列还有要求：使用怎样的拓扑连接和路由策略，然后再计算所有业务此时的带宽分配，按照此路由方案和带宽分配处理业务，直到有通信需求开始或结束；
