...
1. 根据上个重构时间的方案，预测本个重构时间内会出现的流量。一开始的预测不会精确，每个业务的通信时间对之前迭代获得的通信时间取平均，以预测后续的通信时间，每个业务的训练时间看作常数，随着迭代次数的增加，通信时间的预测值也会与实际越来越相近（？还是预测值大头为上个迭代的通信时间？按理来说相邻迭代时间应该近似，那就换个滤波器，改成。）：
