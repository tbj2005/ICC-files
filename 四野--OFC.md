...
由于带宽，流量大小都是对算法可见的，因此可以预测到每个重构时间内网络的所有动作，并能预测到下个重构时间的流量情况。

1. 根据上个重构时间的方案，预测本个重构时间内会出现的流量和这些业务流量出现的次数。一开始的预测不会精确，每个业务的通信时间对之前迭代获得的通信时间取平均，以预测后续的通信时间，每个业务的训练时间看作常数，随着迭代次数的增加，通信时间的预测值也会与实际越来越相近。还是预测值大头为上个迭代的通信时间？按理来说相邻迭代时间应该近似，那就换个滤波器，改成指数级，计算还方便，计算方法为递推：第二次使用第一次的通信时间，第三次使用前两次通信时间的均值，第四次使用该均值和通信时间的均值，本质上就是参数用2的指数分之1来做滤波，计算上更加方便。据此方案获得预测后流量特征，然后以此确定；

2. 进行第一个拓扑重构时间内的调度：在业务通信需求开始时确定是否使用在网聚合：找到这个业务在前面算法获得的环拓扑，并检查网络中是否存存在这样的拓扑，如果不存在，那就直接放进非在网聚合队列；反之，如果存在，也要判断放进那个队列更快：。，使用怎样的拓扑连接和路由策略，然后再计算所有业务此时的带宽分配，按照此路由方案和带宽分配处理业务，直到有通信需求开始或结束；
