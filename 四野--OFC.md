...
静态场景下存在一批业务，已知其数据量和参数量，确定为 GPU 分配的计算资源，使用多阶段联合调度的方法调度它们。

为了简化流的调度过程，使得静态场景下流量按照特定的周期以同种方式循环，这样就可以大大减小调度复杂度。在实际的训练过程中，业务的通信和训练是分开的，这会导致 GPU 在这段时间空闲的问题。为了解决这一问题，假设流量每循环一次为一轮，将一轮分为若干组，每组中各自保留一个固定的 OXC 拓扑，控制业务的计算资源，使其在固定的组内做通信，在剩余的组做训练。当业务在做通信时，腾出来的 GPU 可以用于其他业务做训练，这样可以大幅降低 GPU 空置率。

1. 双阶段，拓扑自己选定---解耦拓扑与业务部署，与光网络似乎关系不大：

将一轮分成两组，即双阶段。第一轮启动后，一阶段开始，为一部分业务分配 GPU 资源，启动训练，在二阶段开始前第一部分业务完成第一次迭代中的训练，一阶段所有业务进入通信模式，并在下一轮开始前完成通信。同时，为二阶段的业务分配一阶段腾出的 GPU 进行训练，并在下一轮开始前完成训练。第二轮开始，启动第一部分业务的训练和第二部分业务的通信。。。。后续的训练过程与第二轮完全相同，这样，流的调度复杂度就被大大简化了。

输入：业务的参数量和 batch size 大小，单个 GPU 的计算能力，网络特征（pod 数目，server pool 大小），二阶段的拓扑，每个业务的数据并行度；

输出：业务计算资源分配方式（部署），和业务时间分配。

1. 根据参数量预测流量大小，根据 batch size 和 GPU 运算能力求单 GPU 训练时间；

2. 按参数量均分业务为前后两组，使用所有 GPU ，按平均化训练时间的方式确定业务 GPU 使用数目，然后根据并行度确定单节点 GPU。使用特定部署方案，根据拓扑连接，选择业务部署方式和业务拓扑（ring/ps）。然后根据获得的方案分别计算两组业务的通信时间。并将第一组业务的通信时间与第二组业务的训练时间比较，将第二组业务的通信时间与第一组业务的训练时间比较。如果都更小，此时 GPU 没有空闲，通信时间不会拖慢整体训练进度，通信气泡对结果无影响，可以输出部署方案；如果两次比较结果不同，说明训练过程出现气泡，此时需要通过交换业务来解决：

3. 优化初始解：每次只交换一个业务，并重新计算它们的解，找到总时间最低的方案（爬山）；

4. 两组通信都出现气泡，此时还是依靠爬山法，找出最优解。

问题：PS 架构在不使用 INC 时，时间不好用流量矩阵来算，因为两段流量是分离的，不像 ring 那样，各流量都是同时传输的。这就导致 ring 和 ps 放到一起算时算不出来。人为产生的拓扑不容易找到环之类的架构，因此希望拓扑相对简单，也就是一个拓扑是由多个简单且无重叠的子拓扑组成。如果子拓扑按照该方式产生，ps 架构无 INC 的时间计算不会和 ring 杂糅到一起，也不再有刚才的问题。

2. 三阶段甚至 n 阶段，拓扑自己选定：

将一轮分成 n 组，具体的方案其实也类似二阶段。一个业务的通信过程会占用 n - 1 组的时间，训练过程会占用一组的时间；

3. 双阶段，算法求拓扑：

算法类似拓扑给定，和我们初始的算法甚至更像，此时 PS 和 ring 结构会杂糅到一起，此时使用 TON 中的二分时间法可以很好解决传输时间的计算问题。



1. 根据业务参数确定流量和单 GPU 训练用时；

2. 平均两组 batch size * 参数量 ，然后分别按满 GPU 分配来给业务分配计算资源，使其尽量平均化时间。然后使用已有算法算拓扑，注意有流量的拓扑至少要一根连接。然后将业务填入拓扑，计算流量矩阵，除连接矩阵后求得通信时间，后面的爬山法就是一样的了。

还有问题，就是聚合时间这件事，聚合会占用 GPU 相关资源，，，AllToAll没有聚合不用管，希望放宽聚合资源限制，使得 ring、ps都没有聚合时间，以再次简化问题。



由于拓扑不仅仅是环，还可能会有 ps 甚至 AllToAll所需的网状结构，此时业务部署算法需要进行一些修改。



目标是最终的训练时间最低。

1. 按业务流量将所有业务分成两份，即主要流量和次要流量；

2. 主要流量按平均化输出流量为主，次要流量按减少 OXC 端口用量来放。ring 结构算法已经确定，然后是 PS 结构。平均化输出流量：先放ps后放worker，然后按输出流量大小放。减少 OXC 端口用量：找同样的拓扑部署。Mesh 结构同理。

3. 输出拓扑和放置方案。



总算法（mountain climb算法）：

1. 初始化两个集合A，B，用以分别存放两组业务，每一组对应一种正交的调度模式。首先将所有业务放进集合 A 中，并置空集合 B 。将整个调度中的每轮分为两组，分别为 $\alpha$ 组和 $\beta$ 组，在 $\alpha$ 组会执行 A 集合业务的训练和 B 集合业务的通信，在 $\beta$ 组会执行 A 集合业务的通信和 B 集合业务的训练。在初始化情况下，问题变成无交叉情况，即在 $\alpha$ 组执行对所有业务的集合训练，对 $\beta$ 组执行对所有业务的集合通信，这就作为业务分组初始的可行解；

2. 按 GPU 完全使用的方式，平均化所有业务的单次训练用时，存储全部业务单次训练用时为 $t_{\alpha}$，以此确定每个业务的 GPU 用量，并使用已有部署和拓扑计算算法来确定拓扑和业务单跳传输方案，并通过二分时间的办法，获得全部业务传输加聚合的总集合通信时间 $t_{\beta}$，单轮用时即为 $t_{round}=t_{\alpha}+t_{\beta}$，初始场景由于没有任何重叠，单轮时长是所有业务分组最长的；

3. 使用爬山法迭代可行解，具体的方案为：遍历每一个业务，假设交换该业务组别然后使用二分求解法（算法二）计算交换后单轮用时，找到单轮用时降低最多的交换方案；

4. 当发现交换任意业务组别后得到的单轮用时增加，此时输出业务分组和拓扑。

算法2：已知业务组别，求解单轮用时算法：

1. 由于已知业务并行度和流量大小，此时可以使用已有部署方案，部署分别求出 A, B 组的预计拓扑和部署方案，然后就可以通过 TON 中的二分时间法求得集合通信时长。然后通过时长判断各组 GPU 是否全部使用，如果需要全部用完，那就全部打满，然后再根据 GPU 分配重新再算一遍通信时间，比较后确定每组用时。