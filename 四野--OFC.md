...
1. 网络特征及各项参数：三层 clos 脊叶网络，ToR 带宽，oxc 连接数目;

2. 每个业务的并行方案和各层并行度，业务通信需求；

3. 交换机聚合器资源；

#### 待解决的问题：

1. 初始业务 GPU 部署问题，将 GPU 部署进网络后怎样部署各并行单元；

2. 为每个业务的通信计算两种方案，分别是使用在网聚合和不使用在网聚合；

3. 在在线模式预测业务的通信需求，并为其确定通信方案，若使用在网聚合，确定使用的聚合器资源，若不使用，则确定网络拓扑和带宽分配，确定是否使用 oxc 重构。在通信过程中，不同迭代下的通信方案可以不同，用以适应不同场景。

4. 拓扑重构触发条件：有业务完成或有业务加入，导致网络中流量倾斜方式变化，此时需要重构以适配之。
