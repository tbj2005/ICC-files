...
在每个重构时间内的调度不使用更细的时隙，而是通信开始或完成都会直接让算法直到，然后马上计算修改带宽分配，并根据带宽分配预测下一个网络状态变化点。
