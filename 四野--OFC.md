...
7. 在网聚合和 AllReduce 算法都可以使用双层操作。在网聚合方面，在同一源 ToR 下先聚合一次，然后不同的 ToR 计算的结果再经过 OXC 到达目的 ToR 下再进行第二次聚合，聚合后流量再次按原方向组播。ring-AllReduce 算法可以在同 ToR 下进行一次 All-Scatter 操作，然后再在 ToR 间再做一次 All-Scatter，然后按反过程再 All-Gather 两级即可。如果只看在同一 ToR 下的 ring 和在网聚合，会发现 ring 的通信量几乎是在网聚合的两倍，而且还有大量的计算时间。如果我们将视线扩展到第二层操作，会发现由于 oxc 全双工的假设，使用在网聚合可以同时用到一条连接两个方向的带宽，但是 ring 只用到了一个方向的带宽，带宽利用率其实是 50%。在 ToR 和 server pool 间不会有这一问题，只有 oxc 部分会出现该问题。不过可以稍微改良一下 ring 算法，使得其可以完全使用两个方向的带宽，即把要传输的数据分成正反两份，分别做不同方向的 ring 就可以了。但是在网聚合会在聚合处产生带宽瓶颈，带宽占用并不是均匀的。

8. 在网聚合需要使用聚合器资源，如果使用双层聚合，会使用更多 ToR 上的带宽资源，假设网络中的聚合器资源不能完全满足所有任务的通信需求，此时我们不得不使用 ring 算法。按理来说，每个业务使用的聚合器资源应该和带宽分配匹配，防止因为计算速度过慢导致拖带宽的后腿；

9. PP 的流量是在训练过程产生的，使用 GPipe 操作后，每层间的前向传播时间不是错开的，而是通过 microbatch 进行了流水化，传输的是每层输出的结果；DP 的流量是各数据并行单元训练完毕后进行的，传输的是模型梯度；EP 的流量是 token ，在专家并行单元间会进行一来一回两次传输，第一次传输是训练完成后还回 token，第二次传输是训练开始前交换 token。可以看到这些流量传输内容是不同的，只看 EP 和 DP ，可以发现这些都是在每次计算结束后进行的，可以进行合并，使其同时进行。而 PP 流量涉及层间通信，不好和其他通信合并。为了简化问题，将 PP 的通信时间放进 GPU 训练过程内，也就是说，在训练过程中还是有通信需求，必须为流水并行组间分配带宽以满足其传输要求。在完成训练后，可以释放这部分带宽，供其他过程使用。
...
1. 判断交换机自身是带宽瓶颈还是聚合器瓶颈，判断方法为：假设 ToR 与 server pool 间的带宽用满，是否会产生聚合器资源瓶颈；假设 OXC 端口与 server pool 间的带宽用满，是否会产生聚合器资源瓶颈，找到哪项资源三项资源中的瓶颈；
...
5. 首先确定最优情况下的二层聚合连接量 N 。若数据并行单元数目为 M ，每个 ToR 上连接数为 C。如果是聚合器资源瓶颈，那么一层聚合和二层聚合应当尽量聚合数据量相同。那么 $N=\lceil\sqrt{M}\rceil$。每个 ToR 还需要保留两根连接以用于 ring 结构的传输，因此 $N=\min\{\lceil\sqrt{M}\rceil,C-2\}$；如果是带宽资源瓶颈，若 ToR 上的 OXC 带宽：server pool 带宽 = K，那么$M=KN^2$，考虑到 ring 的连接，$N=\min\{\lceil\sqrt{\dfrac{M}{K}}\rceil,C-2\}$。a. 在开始部署时， server pool 资源足够，先按照 $ N $ 的大小为业务排序，按降序逐个部署业务。找空的 ToR ，按照最优方式部署。
