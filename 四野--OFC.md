...
5. 首先确定最优情况下的 ToR 使用数目。如果是聚合器资源瓶颈，那么说明我们的部署理想情况下只会用满聚合器资源，此时单个 ToR 下的 DP 单元数目越少，则可以调用的聚合器资源越多；如果是带宽资源瓶颈，若 ToR 上的 OXC 带宽：server pool 带宽 = 1:K，那么说明一个 ToR 下可以部署 $\lfloor K \rfloor$ 个 ToR。a. 在开始部署时， 先按照 $ N $ 的大小为业务排序，按降序逐个部署业务。若每个 ToR 上的 server pool 资源为 S，并行单元消耗 server pool 资源量为 U，如果 $NS \geq UM$，说明 N 个 ToR 够放这个业务了，找 N 个空的 ToR ，然后逐个部署每个 DP 单元，即将其部署在已部署 DP 单元最少的 ToR，直到部署完成，如果部署过程中发现 DP 单元已经无法整个部署在 ToR 上，则将 DP 单元拆分成 moe 层，然后在 ToR 上逐个部署 moe 层，如果剩余的 moe 层已经无法部署进这个 ToR 了，再看 moe 层中的各专家 GPU 是否可以再部署在这个 ToR 上，如果可以部署，那就部署到用完这个 ToR 的剩余资源为止，然后继续看下一个资源剩余最多的 ToR，先放没有被完全部署的 moe 层，然后再按照上述方式部署，直到业务被完全部署。反之，如果$NS<UM$，说明 N 个 ToR 完全放不下这个业务，此时判断$(C-2)S<UM$是否成立，如果否的话，说明 $C-2$个 ToR 已经足够部署了，此时令$N=\lceil \dfrac{UM}{S}\rceil$，按照上述方案放 DP 并行单元即可；如果成立，说明 $C-2$ 个 ToR 还是放不下，再判断$CS<UM$是否成立，如果不成立，令$N=\lceil\dfrac{UM}{S}\rceil$，标记之，按照上述方案部署 DP 并行单元，如果成立，
